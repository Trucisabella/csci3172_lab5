...
function addCreature() {
    ...
    createSanctuary.push(newCreature);
...
function displayCreatures() {
    ...
    for (const creature of createSanctuary) {
    ...
    console.log(creaturesDiv);
}

// Extend functionality (clear the sanctuary list)
function clearSanctuary() {
    createSanctuary.length = 0;
    const clearDiv = document.getElementById('clearSanctuaryDiv');
    clearDiv.innerHTML = '';
    displayCreatures();
}
...
document.getElementById('addCreatureForm').addEventListener('submit', function (event) {
    ...
    const clearDiv = document.getElementById('clearSanctuaryDiv');
    clearDiv.innerHTML = '<button id="clearSanctuaryBtn" class="btn btn-danger">Clear Creature Sanctuary List</button>';
    document.getElementById('clearSanctuaryBtn').addEventListener('click', clearSanctuary);
    addCreature();
});
